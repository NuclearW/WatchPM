...
public class WatchPmPermissionsHandler {
	public static WatchPm plugin;
	
	public WatchPmPermissionsHandler(WatchPm instance) {
		plugin = instance;
	}
	...
	public static void initialize(Server server) {
		...
		if(PEXtest != null) {
			...
			plugin.log.info("[WatchPM] PermissionsEx found.");
		} else if(test != null) {
			PHandle = (PermissionHandler) ((Permissions) test).getHandler();
			permissionType = PermissionType.PERMISSIONS;
			permissionsEnabled = true;
			plugin.log.info("[WatchPM] Permissions "+test.getDescription().getVersion()+" found.");
		} else {
			//plugin.log.info("[WatchPM] Permissions not found, falling back to ops.");
			permissionType = PermissionType.BUKKIT;
			permissionsEnabled = true;
			plugin.log.info("[WatchPM] Using Bukkit Permissions.");
		}
	}
	...
	public static void onEnable(Plugin pluginEnabled) {
		...
		if(pluginName.equals("PermissionsEx")) {
			...
			permissionType = PermissionType.PERMISSIONS;
			permissionsEnabled = true;
			plugin.log.info("[WatchPM] Permissions "+pluginEnabled.getDescription().getVersion()+" found.");
		}
	}
